...
if test_mode == "📥 从列表页批量爬取":
    ...
    **使用说明：**
    ...
    list_url = st.text_input(
        "列表页URL",
        value="https://hangqing.zhuwang.com.cn/doupo/list-67-1.html",
        placeholder="输入豆粕价格列表页URL"
    )
...
# ==================== 查看最新文件 ====================
    ...
    if st.button("🔄 刷新", use_container_width=True):
        ...
        if all_files:
            st.success(f"✅ 找到 {len(all_files)} 个豆粕价格文件")
            ...
            st.warning(f"未找到豆粕价格文件（目录: {spider.save_dir}）")
...
st.markdown("---")
st.caption(f"🌽 豆粕价格爬虫 | 更新时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
